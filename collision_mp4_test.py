...
SCALE_FACTOR             = 15.0      # pixel depth → meters
TTC_THRESHOLD            = 3.0       # seconds
MIN_SPEED                = 0.2       # m/s
MIN_CONF                 = 0.5       # higher confidence
MIN_AREA                 = 1500      # px²
WARNING_PERSIST_FRAMES   = 8         # frames before drawing
MAX_DISTANCE_CAP         = np.inf
VELOCITY_CHANGE_TH       = 2.0       # m/s sudden Δv

# New: distance‐only warning threshold
...
def led_on():
    print("LED ON")    # replace with GPIO.HIGH

def led_off():
    print("LED OFF")   # replace with GPIO.LOW
...
# history buffers
dist_history = {}  # tid → deque[(t, dist)]
vel_history  = {}  # tid → deque[(t, v)]
...
while True:
    ...
    if frame_count % DETECT_INTERVAL == 0:
        ...
        # depth at half-res, then upsample
    ...
    warnings = []  # entries: (tid, ttc_or_None, (x1,y1,x2,y2), reason)
    now      = time.time()
    ...
    if results is not None:
        for box in results.boxes:
            ...
            # bottom-center sampling for distance
            xc = (x1 + x2)//2
            ...
            # distance-only warning
            if dist_m < DISTANCE_THRESHOLD:
                warnings.append((tid, None, (x1,y1,x2,y2), 'DISTANCE'))
    ...
    # ─── draw warnings only after stable debounce ───────────────────────────────
    if warning_counter >= WARNING_PERSIST_FRAMES:
        for _, _, (x1,y1,x2,y2), reason in warnings:
            cv2.rectangle(frame, (x1,y1), (x2,y2), (0,0,255), 2)
            label = "DISTANCE WARNING" if reason=='DISTANCE' else "COLLISION WARNING"
